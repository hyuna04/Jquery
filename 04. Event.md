...
선택자.trigger.("keydown|keyup|keypress", 콜백함수);
//keydown->소문자 대문자 구분X(어떤 키가 눌렸는지 알 수 있음)
//keypress->소문자 대문자 구분O(눌린 키값)
```

## 6. 그룹 이벤트 등록
1. Jquery에서는 그룹 이벤트 등록 기능을 통해서 여러 이벤트에 동시에 같은 기능을 정의 할 수 있다.
